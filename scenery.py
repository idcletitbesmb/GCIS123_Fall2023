...
def main():
    ...
    while height < cake_height:
            color = input('Input the color of the next layer: ') #user sets color for new layer
            if layers_count == 4: # we decided to set the maximum number of layers for 5, so if on this if-check the number of layers is 4, it means that there is only one left
                layer(cake_height - height, cake_widht, color) #draws a layer of the remaining height
                break
            else:  
                new_layer_height = int(input('Input the height of the next layer from 1 to ' + str(cake_height - height) + ': ')) #user inputs height of the next layer from 1 to maximum remaining
                ...
                layers_count += 1 
                layer(new_layer_height, cake_widht, color) #draws a new layer
    update()
    exitonclick()
